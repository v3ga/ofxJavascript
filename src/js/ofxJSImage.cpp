...
JSFunctionSpec ofxJSImage::_JSFunctionSpec[] = {
	...
	{ "getPixelAt", JSFUNC_getPixelAt, 2, 0, 0 },
...
JSBool ofxJSImage::JSFUNC_getPixelAt(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval) {
	ofxJSImage *p = (ofxJSImage*)JS_GetPrivate(cx, obj);
	if (argc < 2) return JS_FALSE;
	if (argc == 2) {
		/* Function: int					getPixelAt(int x, int y) */
		if (JSVAL_IS_NUMBER(argv[0]) && JSVAL_IS_NUMBER(argv[1])) {
			*rval = __int_TO_JSVal(cx,p->getPixelAt(
				__JSVal_TO_int(argv[0]),
				__JSVal_TO_int(argv[1])
			));
			return JS_TRUE;
